...
import {
  Star, Plus, Bell, Target, StickyNote, TrendingUp, TrendingDown,
  Pencil, Trash2, X, Loader2, MoreHorizontal,
} from "lucide-react";
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Investment thesis, why you're watching this..."
          rows={3}
...
function WatchlistItemCard({
  ...
  return (
    <motion.div
      ...
      className="bg-surface border border-border rounded-xl p-4 hover:border-border-subtle hover:shadow-card-hover transition-all group"
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={onRemove}
            className="p-1.5 rounded-lg text-muted hover:text-loss hover:bg-loss-dim transition-colors"
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {item.target_price && (
            <div className="flex items-center gap-1 text-2xs text-accent bg-accent-glow border border-accent/20 rounded-md px-2 py-0.5">
              ...
              Target @ <span className="font-mono">{Number(item.target_price).toFixed(2)}</span>
            </div>
          )}
          {item.alert_price && (
            <div className="flex items-center gap-1 text-2xs text-warn bg-warn-dim border border-warn/20 rounded-md px-2 py-0.5">
              <Bell className="w-2.5 h-2.5" />
              Alert @ <span className="font-mono">{Number(item.alert_price).toFixed(2)}</span>
            </div>
          )}
        </div>
      )}
      ...
      {!item.target_price && !item.alert_price && !item.notes && (
        <p className="text-2xs text-muted italic">No target or notes set</p>
      )}
    </motion.div>
  );
}
        ...
        {showMenu && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 top-full mt-1 z-20 bg-surface-2 border border-border rounded-xl shadow-card-hover py-1 min-w-[140px]"
            >
              <button
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-xs text-loss hover:bg-loss-dim transition-colors"
              >
                <Trash2 className="w-3.5 h-3.5" />
                Delete list
              </button>
...
export default function WatchlistPage() {
  ...
  async function handleAddTicker(data: CreateWatchlistItem) {
    ...
    try {
      ...
      setWatchlists((prev) =>
        prev.map((w) => (w.id === activeId ? { ...w, items: [...w.items, item] } : w))
      );
  ...
  async function handleRemoveItem() {
    ...
    try {
      ...
      setWatchlists((prev) =>
        prev.map((w) => ({
          ...w,
          items: w.items.filter((i) => i.id !== removeItem.id),
        }))
      );
      setRemoveItem(null);
  ...
  return (
    <AppShell
      title="Watchlist"
      subtitle={`${totalItems} items across ${watchlists.length} lists`}
    >
      <div className="p-5 max-w-[1600px] mx-auto">
        {loading ? (
          <div className="flex items-center justify-center py-24 gap-3">
            <Loader2 className="w-5 h-5 text-muted animate-spin" />
            <p className="text-sm text-muted">Loading watchlists...</p>
          </div>
        ) : (
                ...
                {watchlists.map((wl) => (
                  ...
                  >
                    <ListButton
                      ...
                      onRename={() => { setShowRename(wl); setRenameName(wl.name); }}
                      onDelete={() => setDeleteList(wl)}
                    />
                  </motion.div>
                ))}
              </AnimatePresence>
            ...
            <div className="lg:col-span-3 space-y-4">
              {current ? (
                <>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted">{current.items.length} tickers</p>
                    </div>
                    <Button
                      variant="secondary"
                      icon={<Plus />}
                  ...
                  {current.items.length === 0 ? (
                    <div className="text-center py-20 border border-dashed border-border rounded-2xl">
                      ...
                      <Button variant="secondary" icon={<Plus />} onClick={() => setShowAddTicker(true)}>
                ...
                <div className="text-center py-24">
                  ...
                  <Button variant="secondary" icon={<Plus />} onClick={() => setShowNewList(true)}>
                    Create first list
                  </Button>
                </div>
              )}
      ...
      <Modal open={showNewList} onClose={() => setShowNewList(false)} title="New Watchlist" size="sm">
        <div className="space-y-4">
          <input
            ...
            placeholder="e.g. High Conviction, UK Opportunities..."
            autoFocus
            className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-primary placeholder:text-muted focus:outline-none focus:ring-1 focus:ring-accent/40"
          />
      ...
      <Modal open={!!showRename} onClose={() => setShowRename(null)} title="Rename List" size="sm">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button variant="primary" size="sm" loading={renamingSaving} onClick={handleRenameList}>
              Save
            </Button>
          </div>
        </div>
        ...
        {deleteList && (
          <div className="space-y-4">
            <p className="text-sm text-secondary">
              Delete <span className="font-semibold text-primary">"{deleteList.name}"</span> and all {deleteList.items.length} tickers in it? This cannot be undone.
            </p>
            <div className="flex justify-end gap-3">
              <Button variant="ghost" size="sm" onClick={() => setDeleteList(null)}>Cancel</Button>
              <Button variant="danger" size="sm" loading={deletingList} onClick={handleDeleteList}>
                Delete list
              </Button>
            </div>
          </div>
      ...
      <Modal open={showAddTicker} onClose={() => setShowAddTicker(false)} title={`Add ticker to "${current?.name}"`} size="md">
        <AddTickerForm
          onSave={handleAddTicker}
          onCancel={() => setShowAddTicker(false)}
          saving={addingTicker}
        />
      </Modal>
      ...
      <Modal open={!!removeItem} onClose={() => setRemoveItem(null)} title="Remove Ticker" size="sm">
        {removeItem && (
          <div className="space-y-4">
            <p className="text-sm text-secondary">
              Remove <span className="font-mono font-semibold text-primary">{removeItem.symbol}</span> from this watchlist?
            </p>
            <div className="flex justify-end gap-3">
              <Button variant="ghost" size="sm" onClick={() => setRemoveItem(null)}>Cancel</Button>
              <Button variant="danger" size="sm" loading={removingItem} onClick={handleRemoveItem}>
                Remove
              </Button>
            </div>
          </div>
