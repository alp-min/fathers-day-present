...
type SortKey = keyof Pick<Position, "name" | "marketValue" | "unrealisedPL" | "unrealisedPLPct" | "dayChangePct" | "weight" | "beta">;
  ...
  const sorted = useMemo(() => {
    let rows = enriched.filter((p) => {
      ...
      return (
        ...
        (dirFilter === "all" || (p.direction ?? "long") === dirFilter)
      );
    });
    rows = [...rows].sort((a, b) => {
    ...
    return rows;
  }, [enriched, sortKey, sortDir, query, assetFilter, dirFilter]);
  ...
  return (
    <div className="space-y-3">
              ...
              <tr className="border-b border-border">
                {cols.map((col) => (
                  <th
                    ...
                    className={cn(
                      "px-4 py-3 text-muted uppercase tracking-wider font-medium cursor-pointer hover:text-primary transition-colors whitespace-nowrap",
                ...
                <th className="px-4 py-3 text-right text-muted uppercase tracking-wider font-medium">Region</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        {pos.logoUrl ? (
                          <img src={pos.logoUrl} alt={pos.name}
                        ...
                        <div>
                          <div className="flex items-center gap-1.5">
                            ...
                            {isShort && (
                              <span className="text-2xs font-mono text-loss bg-loss/10 px-1 rounded border border-loss/20">S</span>
                            )}
                          </div>
                          <p className="text-muted text-2xs truncate max-w-[110px]">{pos.name}</p>
                          {pos.account && pos.account !== "General" && (
                            <p className="text-2xs text-accent/70 truncate max-w-[110px]">{pos.account}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      <p className={`text-2xs font-mono ${isUp ? "text-gain" : "text-loss"}`}>
                        {isUp ? "+" : ""}{pos.unrealisedPLPct.toFixed(2)}%
                      </p>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      {pos.beta != null ? (
                        <span className={`font-mono text-xs ${
                          pos.beta > 1.5 ? "text-loss" : pos.beta < 0.5 ? "text-gain" : "text-primary"
                        }`}>
                          {pos.beta.toFixed(2)}
                        </span>
                      ) : (
                    ...
                    {/* Region */}
                    <td className="px-4 py-3 text-right">
                      <Badge variant="ghost">{(pos.geography ?? "—").split(" ").slice(0, 2).join(" ")}</Badge>
                    </td>
      ...
      <p className="text-2xs text-muted px-1">
        ...
        {isLive ? "Prices live via Twelve Data" : "Prices from last import"}
      </p>
    </div>
  );
}
